...
export function EditorLayout({ children }: EditorLayoutProps) {
  ...
  const navigation = [
    { name: i18n.t('menu.assignedToMe'), href: '/editor/dashboard', icon: FileText },
    { name: i18n.t('menu.messaging'), href: '/editor/mensajeria', icon: Mail },
    { name: i18n.t('menu.myProfile'), href: '/editor/profile', icon: User },
  ];
        ...
        <div className="p-6 border-b border-gray-200 bg-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">{i18n.t('layout.editorPanel')}</p>
            </div>
          </div>
        </div>

        <nav className="flex-1 p-4 space-y-6 overflow-y-auto h-[calc(100vh-240px)]">
          {/* Sección: Gestión editorial */}
          <div>
            <h3 className="px-3 text-xs font-semibold text-gray-900 uppercase tracking-wider mb-2">
              {i18n.t('menu.editorialManagement')}
            </h3>
            <Link
              to="/editor/submissions"
              onClick={() => setSidebarOpen(false)}
              className={`block px-3 py-2 text-sm transition-colors ${
                isActive('/editor/submissions') || isActive('/editor/dashboard')
                  ? 'font-semibold'
                  : 'text-gray-700 hover:text-gray-900 hover:bg-gray-100'
              }`}
              style={{
                color: isActive('/editor/submissions') || isActive('/editor/dashboard') ? FESC_RED : undefined,
              }}
            >
          ...
          <div>
            ...
            <Link
              to="/editor/statistics/articulos"
              onClick={() => setSidebarOpen(false)}
              className={`block px-3 py-2 text-sm transition-colors ${
                isActive('/editor/statistics/articulos')
                  ? 'font-semibold'
                  : 'text-gray-700 hover:text-gray-900 hover:bg-gray-100'
              }`}
              style={{
                color: isActive('/editor/statistics/articulos') ? FESC_RED : undefined,
              }}
            >
              {i18n.t('menu.articles')}
            </Link>
            <Link
              to="/editor/statistics/revista"
              onClick={() => setSidebarOpen(false)}
              className={`block px-3 py-2 text-sm transition-colors ${
                isActive('/editor/statistics/revista')
                  ? 'font-semibold'
                  : 'text-gray-700 hover:text-gray-900 hover:bg-gray-100'
              }`}
              style={{
                color: isActive('/editor/statistics/revista') ? FESC_RED : undefined,
              }}
            >
          ...
          <div>
            ...
            <Link
              to="/editor/mensajeria"
              onClick={() => setSidebarOpen(false)}
              className={`block px-3 py-2 text-sm transition-colors ${
                isActive('/editor/mensajeria')
                  ? 'font-semibold'
                  : 'text-gray-700 hover:text-gray-900 hover:bg-gray-100'
              }`}
              style={{
                color: isActive('/editor/mensajeria') ? FESC_RED : undefined,
              }}
            >
          ...
          <div>
            ...
            <Link
              ...
              style={{
                color: isActive('/editor/profile') ? FESC_RED : undefined,
              }}
            >
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-xl font-semibold text-gray-900">
                {(location.pathname === '/editor/dashboard' || location.pathname.startsWith('/editor/submissions')) && i18n.t('page.editorialManagement')}
                {location.pathname.startsWith('/editor/statistics') && i18n.t('page.statistics')}
                {location.pathname.startsWith('/editor/mensajeria') && i18n.t('page.messaging')}
                {location.pathname.startsWith('/editor/profile') && i18n.t('page.myProfile')}
              </h1>
            </div>
            <div className="flex items-center gap-4">
              ...
              >
                <div className="text-right">
                  ...
                  <p className="text-xs text-gray-500">{i18n.t('role.editor')}</p>
                </div>
